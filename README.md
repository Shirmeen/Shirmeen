<div align="center">

# Shirmeen Aamir 
### **Generative AI Engineer • Deep Learning Practitioner • Systems Builder**

<a href="https://shirmeen.github.io/Shirmeen/"><img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=24&duration=3000&pause=1000&color=00F0FF&center=true&vCenter=true&width=600&lines=Hi+there%2C+I'm+Shirmeen!+%F0%9F%91%8B;Generative+AI+Engineer;Deep+Learning+and+Computer+Vision;Always+learning%2C+always+building!+%F0%9F%9A%80" alt="Typing SVG" /></a>

<br/>

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/shirmeen-amir-35ab81264)
[![Gmail](https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:shirmeenaamir112@gmail.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-bd00ff?style=for-the-badge&logo=react&logoColor=white)](https://shirmeen.github.io/Shirmeen/)

</div>

---

### 🚀 About Me
I am a Data Scientist and AI Engineer specializing in **Generative AI (GANs, VAEs, Stable Diffusion)**, **Deep Learning**, and **Computer Vision**. I design intelligent systems using state-of-the-art architectures, uncertainty-quantified Graph Neural Networks (GNNs), and automated agentic workflows.

```python
class ShirmeenAamir:
    def __init__(self):
        self.role       = "Associate Generative AI Engineer"
        self.location   = "Lahore, Pakistan"
        self.languages  = ["Python", "C/C++", "SQL", "JavaScript", "Assembly x86"]
        self.domains    = ["Generative AI", "Bayesian GNNs", "Computer Vision", "NLP"]
        self.currently  = "Coordinating tech & creative systems, scaling AI workflows"
        self.fun_fact   = "I've built a full doubly linked notepad & classic games in assembly 🎮"
```

---

### 💼 Work Experience
- **Activity Coordinator** — *Crescent Model Higher Secondary School Girls Campus* (Feb 2026 – Present)
- **Associate Generative AI Engineer** — *BIG IMMERSIVE* (Aug 2025 – Jan 2026)

---

### 🛠️ Core Tech Stack

<div align="center">

| Area | Technologies |
| :--- | :--- |
| **Generative AI & LLMs** | `LangChain` `GANs` `VAEs` `Stable Diffusion` `CLIP` `Prompt Engineering` |
| **Deep Learning & CV** | `PyTorch` `TensorFlow` `Scikit-Learn` `OpenCV` `Bayesian GNNs` `CNNs` |
| **Databases & Frameworks** | `PostgreSQL` `SQL Server` `Docker` `Django` `React` `SvelteKit` |
| **Tools & Platforms** | `Git` `Tableau` `Power BI` `Azure DevOps` `Jupyter` `Linux` |

</div>

---

### 📂 Repository Directory

#### 🤖 AI, Deep Learning & Generative Models
*   **[ADetectPro (FYP)](https://github.com/Shirmeen/fyp)** — Early Alzheimer's detection using Bayesian GNNs with uncertainty quantification. `Python` `GNN` `Deep Learning`
*   **[Generative AI Models](https://github.com/Shirmeen/Generative-Adversarial-Networks-GANs-Autoencoders-AE-Variational-Autoencoders-VAEs-)** — GANs, Autoencoders & VAEs for anomaly detection and generative modeling. `Python` `GANs` `VAEs`
*   **[EmoNet](https://github.com/Shirmeen/EmoNet)** — Emotion analysis via CNN, SVM, and Random Forest for facial expression classification. `Python` `CNN` `SVM`
*   **[AI Chatbot](https://github.com/Shirmeen/Chatbot)** — AI-powered chatbot using NLP techniques. `Python` `NLP` `Jupyter`
*   **[Smart Gaming Picks](https://github.com/Shirmeen/smart-gaming-picks)** — ML-based game recommendation & success prediction engine. `ML` `NLP` `Jupyter`
*   **[Diabetes Prediction](https://github.com/Shirmeen/Diabetes-Prediction-Using-Concept-Hierarchies-and-Clustering)** — Predicts diabetes using concept hierarchies, binning, and clustering. `Clustering` `Jupyter`
*   **[Thermal Comfort](https://github.com/Shirmeen/Thermal-Comfort-Prediction-Using-Machine-Learning-Models)** — Predicts thermal comfort using machine learning regression and classification. `ML` `Regression`

#### 🌐 Web Applications & Platforms
*   **[NOTETAKE5R](https://github.com/Shirmeen/NOTETAKE5R-)** — Note-taking platform with Google Calendar OAuth sync and real-time backend. `SvelteKit` `OAuth` `JS`
*   **[React Vite App](https://github.com/Shirmeen/app)** — Modern React + TypeScript + Vite application with HMR and optimized build. `React` `TS` `Vite`
*   **[Moot 2.0](https://github.com/Shirmeen/moot2.0)** — Web application with a sleek and interactive UI. `HTML` `CSS` `JS`
*   **[Weaver's Den](https://github.com/Shirmeen/Weaver-s-Den)** — Full-stack web app connecting users with textile manufacturers. `JavaScript` `Full Stack`
*   **[AI Workshop](https://github.com/Shirmeen/ai-workshop)** — Interactive AI toolkit website exploring modern AI workflows. `HTML` `CSS` `JS`
*   **[Department of Academics](https://github.com/Shirmeen/department-of-acadamics)** — Educational/Academic layout directory page. `HTML` `CSS`
*   **[CEO App](https://github.com/Shirmeen/ceo-appp)** — Executive dashboard interface setup. `Web`

#### 💻 Systems Programming, Data Structures & Low-Level
*   **[Pacman Game](https://github.com/Shirmeen/Pacman-Game-Implementation-in-C-)** — Classic Pacman in C++ built using Object-Oriented Programming (OOP) and graphics. `C++` `OOP` `Graphics`
*   **[Connect-N Game](https://github.com/Shirmeen/Connect-N-Gam)** — Multi-player Connect-N in C++ with OOP. `C++` `OOP`
*   **[Music Playlist Manager](https://github.com/Shirmeen/Music-Playlist-Manager)** — C++ playlist manager using doubly linked lists. `C++` `Data Structures`
*   **[2D Doubly Linked Notepad](https://github.com/Shirmeen/Project-Implement-a-Notepad-using-a-Two-Dimensional-Doubly-Linkedlist.)** — Notepad built using a two-dimensional doubly linked list. `C++` `Data Structures`
*   **[Tic-Tac-Toe (Assembly)](https://github.com/Shirmeen/Tic-Tac-Toe-in-Assembly-Language)** — Classic game implemented in x86 Assembly language. `Assembly` `x86`

---

### 🎖️ Certifications & Credentials

- 🤖 **Multi AI Agent Systems** — *DeepLearning.AI* (2024) \| [View Certificate ↗](https://learn.deeplearning.ai/accomplishments/b60fc0e8-55fb-4aca-bcbf-5929472d5c89)
- 🎓 **10Pearls University** — *10Pearls* (2024) \| [View Certificate ↗](https://10pearlsuniversity.org/view-certificate/?cid=10PUC-6efc0be387dc98490e8a7165e27cedd46c33724bea620f84195311403)
- 🧠 **Intro to Deep Learning** — *Kaggle* (2024) \| [View Certificate ↗](https://www.kaggle.com/learn/certification/shirmeenaamir/intro-to-deep-learning)
- 🗄️ **Intro to SQL** — *Kaggle* (2023) \| [View Certificate ↗](https://www.kaggle.com/learn/certification/shirmeenaamir/intro-to-sql)
- 📊 **Data Visualization** — *Kaggle* (2023) \| [View Certificate ↗](https://www.kaggle.com/learn/certification/shirmeenaamir/data-visualization)
- 🐍 **Intro to Programming** — *Kaggle* (2023) \| [View Certificate ↗](https://www.kaggle.com/learn/certification/shirmeenaamir/intro-to-programming)

---

### 📈 GitHub Activities & Stats

<div align="center">

![Profile Summary Card](https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=Shirmeen&theme=tokyonight)
![Commit Language Card](https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=Shirmeen&theme=tokyonight)

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=Shirmeen&theme=tokyonight&hide_border=true&ring=00F0FF&fire=ff007f&currStreakLabel=00F0FF)

</div>

<br/>

<div align="center">

🚀 *Let's collaborate and build something remarkable! Feel free to connect.*

```javascript
console.log("Keep inventing! 💡");
```

</div>
